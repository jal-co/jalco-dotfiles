...
Preview content is rendered as markdown in a monospace box. Multi-line text with newlines is supported. When any option has a preview, the UI switches to a side-by-side layout with a vertical option list on the left and preview on the right. Do not use previews for simple preference questions where labels and descriptions suffice. Note: previews are only supported for single-select questions (not multiSelect).`,
		...
		parameters: QuestionParamsWireSchema,

		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			if (!ctx.hasUI) return buildToolResult(ERROR_NO_UI, { answers: [], cancelled: true, error: "no_ui" });

			// LOCAL FORK PATCH — coerce a stringified `questions` payload.
			// Some runtimes deliver the `questions` array JSON-encoded as a string
			// (double-encoded nested tool args). Parse it back before validation so
			// the downstream guards and UI see the structured array they expect.
			const rawQuestions = (params as { questions?: unknown })?.questions;
			let questions: unknown = rawQuestions;
			if (typeof questions === "string") {
				try {
					questions = JSON.parse(questions);
				} catch {
					return buildToolResult(
						"Error: `questions` was a string but not valid JSON. Emit `questions` as an array of question objects.",
						{ answers: [], cancelled: true, error: "no_questions" },
					);
				}
			}
			...
			const typed = { questions } as unknown as QuestionParams;
