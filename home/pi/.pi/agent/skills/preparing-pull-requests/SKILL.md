...
4. Load `ponytail-review` and review the complete diff for avoidable complexity. Apply valid findings, then run applicable existing checks and the repository's anti-slop pass. Stop on unresolved findings, failing checks, missing required screenshots, or missing required human approval. Publish only when the user authorized it. Use GitHub CLI 2.99.0 or later with repeatable `gh --attach` flags and matching local image references in the body. Upload-only assets stay out of Git. Preserve existing issue links and link the PR to the supplied issue.
