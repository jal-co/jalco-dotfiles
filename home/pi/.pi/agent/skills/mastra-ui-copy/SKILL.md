...
| Progress | The -ing verb, no ellipsis | `Deploying`, `Linking branches`, `Loading models`, `Creating` |
...
| Ellipses | None, anywhere in copy: not in progress text, placeholders, pickers, or menu items that open a dialog. Truncating long text with CSS (`truncate`) is layout, not copy, and stays. |
...
- [ ] No ellipses, no em dashes, no exclamation marks; curly apostrophes and quotes
