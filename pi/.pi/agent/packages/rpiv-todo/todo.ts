...
export function registerTodoTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		parameters: TodoParamsWireSchema,

		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			// LOCAL FORK PATCH — coerce JSON-stringified container fields (blockedBy,
			// addBlockedBy, removeBlockedBy, metadata) that some runtimes deliver as
			// strings, before the reducer runs. Throw on malformed input — the tool
			// contract signals failure via throw, not an isError flag.
			const coerced = coerceTodoParams(params as Record<string, unknown>);
			if (!coerced.ok) throw new Error(coerced.error);
			const safeParams = coerced.params as TaskMutationParams;
			const action = (params as { action: Parameters<typeof applyTaskMutation>[1] }).action;
			const result = applyTaskMutation(getState(), action, safeParams);
			commitState(result.state);
			return buildToolResult(action, safeParams, result.state, result.op);
		},
