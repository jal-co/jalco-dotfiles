...
description: "Explicit invocation only: use when the user names make-interfaces-feel-better directly. Design engineering principles for interface polish details (concentric radii, shadows, icon crossfades, press scale, hit areas). Generic polish, \"feels off\", or detail-work requests route to design-engineer-bible (dxb) instead, which has this skill's content distilled into its references."
---
