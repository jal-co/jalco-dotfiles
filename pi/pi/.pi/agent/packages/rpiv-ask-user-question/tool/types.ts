...
/**
 * LOCAL FORK PATCH — wire-level schema tolerant of stringified `questions`.
 *
 * Some pi runtimes / model adapters deliver nested array-of-object tool
 * arguments JSON-encoded as a *string* (double-encoding). With the strict
 * `QuestionParamsSchema`, pre-execute validation then rejects the call with
 * `questions.0: must be object` and the tool is unusable. This schema accepts
 * EITHER the structured array OR a JSON string at the validation gate;
 * `execute()` coerces the string back into an array before running the normal
 * `validateQuestionnaire` guards.
 */
export const QuestionParamsWireSchema = Type.Object({
	questions: Type.Union([
		QuestionsSchema,
		Type.String({
			description:
				"JSON-encoded array of question objects. Fallback for runtimes that stringify nested tool arguments — prefer emitting the structured array.",
		}),
	]),
});
