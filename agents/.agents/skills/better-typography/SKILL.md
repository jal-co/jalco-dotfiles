...
description: >-
  Explicit invocation only: use when the user names better-typography directly or asks a narrow font-format/OpenType/variable-font implementation question (woff2, font-feature-settings, text-box, line clamp). General typography direction, type scale, and pairing decisions route to design-engineer-bible (dxb) instead.
---
